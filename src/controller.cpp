// controller.cpp
// Implementation of the Controller — owns the displays, routes commands,
// loads config, logs events, and runs temperature alerts.
...
static bool isPerDisplayCommand(const std::string& cmd) {
    return cmd == "SET_POWER" || cmd == "SET_VOLUME" || cmd == "SET_BRIGHTNESS" ||
           cmd == "SET_INPUT";
}

Controller::Controller() {
    // Displays are populated later via loadConfig() or addDisplay().
}

void Controller::addDisplay(int id, const std::string& name) { displays.emplace_back(id, name); }
...
void Controller::loadConfig(const std::string& filepath) {
    ...
    while (std::getline(file, line)) {
        ...
        std::string name;
        if (!(iss >> id >> name)) {
            std::cerr << "[WARNING] Skipping malformed line " << lineNumber << ": " << line << "\n";
            continue;
        }
    ...
    if (displays.empty()) {
        throw std::runtime_error("Config file contains no valid displays: " + filepath);
    }

    std::cout << "[SYSTEM] Loaded " << displays.size() << " displays from " << filepath << "\n";
}
...
void Controller::execute(const ParsedCommand& cmd) {
    ...
    // Broadcast: apply the command to every display.
    if (cmd.isAll) {
    ...
    try {
        if (cmd.command == "SET_POWER") {
            ...
            if (cmd.args[0] != "ON" && cmd.args[0] != "OFF")
                throw std::invalid_argument("SET_POWER must be ON or OFF, got: " + cmd.args[0]);
            ...
            try {
                ...
                throw std::invalid_argument("SET_VOLUME expected a number, got: " + cmd.args[0]);
            }
            d.setVolume(val);
            ...
            try {
                ...
                throw std::invalid_argument("SET_BRIGHTNESS expected a number, got: " +
                                            cmd.args[0]);
            }
            d.setBrightness(val);
        ...
        std::cout << "[OK] Display " << d.getId() << " executed: " << cmd.command << "\n";
